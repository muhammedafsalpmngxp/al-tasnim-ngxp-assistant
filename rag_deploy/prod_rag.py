...
# Model used for synthesis (answer generation) when Groq key is available.
# Set GROQ_SYNTHESIS_MODEL in .env to override. Default: llama-3.3-70b-versatile.
...
def _call_llm_synthesis(system: str, user: str, timeout: int = 180) -> str:
    """High-quality LLM call for answer synthesis.
    Uses Groq (llama-3.3-70b — GPT-4o class) when GROQ_API_KEY is set,
    regardless of LLM_PROVIDER. Falls back to Ollama when no key.
    Intent classification still uses the cheap Ollama model via _call_llm()."""
    _has_groq = GROQ_API_KEY and GROQ_API_KEY not in ("", "your_groq_api_key_here")
    if not _has_groq:
        return _call_llm(system, user, timeout)
    ...
    for delay in delays + [None]:
        ...
        if resp.status_code == 429:
            if delay is None:
                return _call_llm(system, user, timeout)  # rate-limited → fall back to Ollama
            import logging
            logging.getLogger("prod_rag").warning("Groq synthesis 429 — waiting %ds", delay)
            time.sleep(delay)
            continue
        resp.raise_for_status()
        return resp.json()["choices"][0]["message"]["content"].strip()
...
def _compile_sql(intent: dict) -> tuple[str, dict]:
    ...
    def build_cols() -> str:
        ...
        if cfg_cols:
            # Only keep columns that actually exist in the live DB — stale YAML entries silently dropped
            valid = [c for c in cfg_cols if c in live_col_map]
    ...
    if sql_type == "range_list":
        ...
        max_val = raw_max / 100 if raw_max > 1 else raw_max
        cols    = build_cols()
        order   = intent_cfg.get("order_by", f"{range_col} ASC")
        return (f"SELECT {cols} FROM {table} "
                f"WHERE {range_col} BETWEEN %(min_val)s AND %(max_val)s "
    ...
    raise ValueError(
        ...
        f"status_breakdown, range, range_list, detail, multi_filter"
    )
...
def _classify_intent(question: str) -> dict:
    ...
    for keyword, intent_name, extract_param in _FAST_PATH:
        if keyword in q_lower:
            ...
            elif extract_param == "well_name":
                # Extract a well name like SWER101_W01 or a numeric ID
                well_id_m = _WELL_ID_RE.search(question)
                if well_id_m:
                    intent["pdo_well_id"] = well_id_m.group(1)
                else:
                    # Match well names: SWER101_W01, NIMR-1659, AMIN_1673188_OP15, etc.
                    wn_m = re.search(r'\b([A-Za-z]{2,}\d*[-_]\w+)\b', question, re.IGNORECASE)
                    if wn_m:
                        intent["well_name"] = wn_m.group(1).upper()
    ...
    # Fast-path: rig count — "how many wells does SWER101 have / on SWER102 / for SWER103"
    # Deterministic so the local LLM never drops the filter_column/filter_value.
    if any(kw in q_lower for kw in ("how many", "count of", "number of", "total wells on", "total wells for")):
        rig_m = re.search(_RIG_ID_PATTERN, q_lower)
        if rig_m:
            intent = {"intent": "well_count",
                      "filter_column": "rig_no",
                      "filter_value": rig_m.group(1).upper()}
            print(f"[sql] Fast-path rig-count: {intent}")
            return intent
...
def sql_answer(question: str) -> dict[str, str]:
    ...
    # Build query context so the LLM knows exactly what was queried and which
    # column each value belongs to — prevents entity misidentification (e.g.
    # calling a rig a "well" because the question said "well").
    intent_name = (intent.get("intent") or "").strip()
    intent_cfg  = _INTENT_CFG.get(intent_name, {})
    sql_type    = intent_cfg.get("sql_type", "")
    table_name  = intent_cfg.get("table", _ALLOWED_TABLE)
    filter_col  = intent.get("filter_column", "")
    filter_val  = intent.get("filter_value", "")

    query_ctx_lines = [f"Table: {table_name}", f"Query type: {sql_type}"]
    if filter_col and filter_val:
        query_ctx_lines.append(f"Filter: {filter_col} = '{filter_val}'")
    if sql_type in ("list", "ranked", "range_list", "multi_filter"):
        query_ctx_lines.append(f"Rows returned: {len(rows)} (limit {_MAX_ROWS})")
    query_ctx = "\n".join(query_ctx_lines)

    # Instruction varies by sql_type so the LLM formats correctly
    if sql_type in ("list", "ranked", "range_list", "multi_filter"):
        format_instruction = (
            "Enumerate each row as a separate item. "
            "Do NOT summarise into a single count — list the individual records."
        )
    elif sql_type in ("count", "count_by_column", "status_breakdown", "group_by"):
        format_instruction = (
            "Report the exact count/breakdown numbers from the result. "
            "Use the column names to correctly identify entities — "
            "rig_no means RIG, pdo_well_id means PDO well ID."
        )
    else:
        format_instruction = "Report only what the data shows."

    system = (
        "You are a strict data reporter for AL TASNIM Enterprises LLC (oil & gas, Oman). "
        "Report ONLY what the database returned — never add, infer, or extrapolate. "
        "Use column names in the result header to identify entities correctly. "
        f"{format_instruction} "
        "Use EXACTLY these 7 headers in this order, each on its own line:\n\n"
        "DIRECT ANSWER:\n"
        "EVIDENCE:\n"
        "SOURCE / CITATION:\n"
        "CONFIDENCE LEVEL:\n"
        ...
        "No markdown, no asterisks, no bullet points. Do not rename or skip any header."
    )
    user = (
        f"Query context:\n{query_ctx}\n\n"
        f"Question: {question}\n\n"
        f"Database results from {source_file}:\n{result_text}\n\n"
        "Report using the 7-section format."
    )
    try:
        raw = _call_llm_synthesis(system, user, timeout=120)
    except Exception:
        raw = ""
    ...
    if raw and not is_refusal:
        ...
        parsed = {
            ...
            "risk_limitation":         f"Shows a maximum of {_MAX_ROWS} rows.",
    ...
    # SQL number guard: if result is a single-row count, verify the exact number
    # appears in direct_answer. If the LLM wrote the wrong number, override with
    # a factual statement built directly from the DB row — zero hallucination.
    parsed = _validate_sql_numbers(parsed, cols, rows, intent)
    return parsed


def _validate_sql_numbers(
    parsed: dict, cols: list[str], rows: list, intent: dict
) -> dict:
    """Verify that count numbers in the answer match the actual DB result.
    For count/status queries: if the LLM wrote a different number, replace
    direct_answer with a statement built directly from the DB row."""
    ...
    # Single-row result with a count column → high risk of hallucination
    if len(rows) == 1 and any("count" in c for c in col_lower):
        db_count = str(rows[0][col_lower.index(next(c for c in col_lower if "count" in c))])
        if db_count not in direct:
            filter_col = intent.get("filter_column", "")
            filter_val = intent.get("filter_value", "")
            if filter_col and filter_val:
                parsed["direct_answer"] = (
                    f"There are {db_count} records where {filter_col} = '{filter_val}'."
                )
            else:
                parsed["direct_answer"] = f"Total count: {db_count}."
            parsed["confidence_level"] = "High — verified directly from database."

    # Multi-row results: if answer says "no records" but rows exist, correct it
    elif len(rows) > 0 and "no record" in direct.lower():
        parsed["direct_answer"] = (
            f"Found {len(rows)} record(s). See evidence for the full list."
        )
        parsed["confidence_level"] = "High — verified directly from database."
...
def llm_answer(query: str, hits: list[Hit]) -> dict[str, str]:
    ...
    raw = _call_llm_synthesis(_SYSTEM_PROMPT, user_msg, timeout=180)
    parsed = _parse_structured_answer(raw)
