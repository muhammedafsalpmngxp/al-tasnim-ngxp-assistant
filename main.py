...
VALUE_TOP_K            = int(os.getenv("VALUE_TOP_K", "6"))
...
# Optional: comma-separated list of table names to EXCLUDE from discovery.
...
def _normalize_type(sa_type: str) -> str:
    """Shorten SQLAlchemy type strings like NVARCHAR(length=255) → nvarchar."""
...
def discover_schema(engine, tables: list[str]) -> dict[str, list[tuple[str, str]]]:
    """
    Returns {table: [(col_name, type_str), ...]} by introspecting the live DB.
    Skips tables listed in DB_EXCLUDE_TABLES. Handles bracketed table names.
    """
    ...
    for table in tables:
        if table not in all_db_tables:
            continue
        if table in DB_EXCLUDE_TABLES:
            logger.debug(f"Skipping excluded table: {table}")
            continue
        try:
...
def _schema_hash(schema: dict) -> str:
    """MD5 of the sorted schema dict — changes when columns are added/removed/renamed."""
    return hashlib.md5(
...
# Queries sys.foreign_keys + column-name matching — nothing hardcoded.
# ============================================================
...
def discover_relationships(engine, schema: dict) -> dict[str, list[tuple]]:
    ...
    # Column-name matching (implicit foreign keys)
...
# Indexes actual distinct values from text columns so the LLM
# sees real data — not guesses — when building WHERE clauses.
# ============================================================
...
def _build_value_documents(engine, schema: dict) -> list[Document]:
    """
    Samples up to VALUE_SAMPLE_LIMIT distinct values per text column.
    Each Document text: "<value> | table:<t> | col:<c>" with metadata for retrieval.
    """
    docs: list[Document] = []
    for table, cols in schema.items():
...
# ============================================================
# TABLE SCHEMA INDEX — vector index over table context strings
# ============================================================
...
def _rich_table_context(table: str, schema: dict, rels: dict) -> str:
    """Single-table context string: columns + relationships (injected into table index)."""
...
# ============================================================
# VALUE GROUNDING — retrieve real DB values matching the question
# ============================================================

def _retrieve_value_hints(question: str, value_retriever) -> str:
    """Returns a formatted block of real DB values relevant to the question."""
...
# ============================================================
# LLM WRAPPER — Groq primary → Ollama fallback, with retry
# ============================================================

def _is_rate_limit(err: Exception) -> bool:
    if hasattr(err, "status_code") and err.status_code in (429, 413):
        return True
    return any(k in str(err).lower() for k in ("rate limit", "429", "413", "quota", "too large"))
...
def _is_transient(err: Exception) -> bool:
    ...
    if hasattr(err, "status_code") and err.status_code in (500, 502, 503):
        return True
    return any(k in str(err).lower() for k in ("timeout", "connection", "temporarily"))
...
def _llm_complete(prompt: str, groq_llm, ollama_llm) -> str:
    ...
    try:
        ...
        if _is_rate_limit(e):
            logger.warning("Groq rate-limited — switching to Ollama.")
...
def _classify_intent(question: str, groq_llm, ollama_llm) -> str:
    ...
    try:
        result = _llm_complete(_INTENT_PROMPT.format(question=question), groq_llm, ollama_llm)
        return "OTHER" if result.strip().upper().startswith("OTHER") else "DATA"
    except Exception:
        return "DATA"

def _chitchat_reply(question: str, groq_llm, ollama_llm) -> str:
    return _llm_complete(_CHITCHAT_PROMPT.format(question=question), groq_llm, ollama_llm)
...
# Produces a structured plan (intent + tables + join paths) before SQL.
# ============================================================
...
def _plan_query(question: str, schema_context: str, groq_llm, ollama_llm) -> tuple[str, dict]:
    """Returns (plan_str, plan_dict). plan_dict is empty if JSON parsing fails."""
    raw = _llm_complete(
...
def _generate_sql(question: str, schema_context: str, value_hints: str, plan: str,
    ...
    sql = _llm_complete(prompt, groq_llm, ollama_llm)
...
def _critique_sql(sql: str, question: str, schema_context: str, groq_llm, ollama_llm) -> str:
...
# ============================================================
# RICH SCHEMA CONTEXT — used in all prompts
# ============================================================

def _schema_context_for_tables(tables: list[str], schema: dict, rels: dict) -> str:
    """Multi-table context string: columns + relationships for each table."""
    blocks = []
    for table in tables:
        cols = schema.get(table, [])
        col_str = ", ".join(f"{c} ({t})" for c, t in cols)
...
def ask(
    ...
    # Step 3: value grounding — fetch real DB values matching the question
    value_hints = _retrieve_value_hints(contextual_q, value_retriever)

    # Step 4: query planner
    plan_str, plan_dict = _plan_query(contextual_q, schema_context, groq_llm, ollama_llm)
    logger.info(f"Plan: {plan_str[:200]}")

    # Step 4b: clarification gate — if user wants ranking but planner found no metric, ask
    ...
    # Step 5: generate SQL
    sql = _generate_sql(contextual_q, schema_context, value_hints, plan_str, groq_llm, ollama_llm)

    # If SQL generator itself signals clarification needed
    ...
    sql = _critique_sql(sql, contextual_q, schema_context, groq_llm, ollama_llm)

    # Step 7: validate (read-only guard fires inside SafeSQLDatabase.run_sql)
    # Step 8: execute
    results_str, _ = sql_db.run_sql(sql)

    # Step 9: synthesize human-friendly answer
...
# # Tables to include — only names, no schema (loaded from DB at runtime)
# TARGET_TABLES = [
...
# # ============================================================
# # DYNAMIC SCHEMA DISCOVERY
# # Fetches table/column/type from the live DB — no hardcoded schema.
# # ============================================================
...
#     """
#     Returns {table: [(col_name, type_str), ...]} by introspecting the live DB.
#     Handles SQL Server bracketed / unusual table names.
#     """
#     inspector = sa_inspect(engine)
#     all_db_tables = set(inspector.get_table_names())
#     schema: dict[str, list[tuple[str, str]]] = {}
...
# # ============================================================
# # INTENT CLASSIFIER — block non-data questions before SQL runs
# # ============================================================
...
# Is the user's input a DATABASE QUERY (asking for data about wells, employees, crews,
# equipment, tasks, revenue, or drilling operations)?
...
# Respond naturally in 2-4 sentences:
# - Greeting → briefly introduce yourself and mention what you can answer (employees, crews,
#   wells, tasks, equipment, revenue, drilling sequences).
# - Farewell → say goodbye warmly.
# - Thanks / compliment → acknowledge and invite further questions.
# - Help / capability question → list your topics with 1-2 examples.
# - Anything else → respond politely and redirect toward data questions.
...
# def _classify_intent(question: str, groq_llm, ollama_llm) -> str:
#     if _NON_DATA_RE.match(question.strip()):
#         return "OTHER"
...
# - ordering rules (CRITICAL):
#   * If the user asks for "top N", "best", "highest", "lowest", "most", "least", "ranked":
#     - Pick the most relevant numeric column from the retrieved tables (e.g., progress, actual_purpose_value, manhours, duration, pms).
#     - Set ordering.column to that column name (EXACT as in schema) and direction to DESC (or ASC for lowest/least).
#     - Prefer columns with names like: progress, actual_progress, acutal_progress, pms, manhours, duration, Normal_duration, over_all_progress_percentages.
#   * If no suitable numeric column exists, set ordering to null — do NOT guess.
#   * If the question does NOT ask for ranking, set ordering to null.
# - Return ONLY valid JSON, no extra text."""
...
#    a. Use LIKE '%value%', never =, for names, labels, or free-text fields.
#    b. SQL Server CI collation handles case — no LOWER() needed.
#    c. For person lookups, search BOTH [Name] AND [Email].
#    d. For partial/misspelled names use the clearest root (e.g. 'rajsh' → '%raj%').
#    e. Use = only for numeric IDs or explicit codes (Well_ID, task_code).
# 7. RANKING — when the question uses "top N", "best", "highest", "lowest", "most", "least":
#    a. Use ORDER BY with the column from the plan's ordering field.
...
#     # Dynamic schema — no hardcoded columns
#     logger.info("Discovering schema from live DB...")
#     schema = discover_schema(engine, TARGET_TABLES)
#     found   = list(schema.keys())
#     missing = [t for t in TARGET_TABLES if t not in schema]
#     if missing:
#         logger.warning(f"Tables not in DB: {missing}")
#     if not found:
#         logger.error("None of the target tables found.")
#         sys.exit(1)
#     logger.info(f"Schema loaded: {len(found)} tables, "
#                 f"{sum(len(v) for v in schema.values())} columns total.")
...
#     total_rels = sum(len(v) for v in rels.values())
#     total_vals = sum(
#         1 for t in schema.values() for _ in t
#     )
#     print("\n" + "=" * 70)
#     print("DB ASSISTANT — ready")
#     print(f"  Primary LLM  : {GROQ_MODEL_NAME}")
#     print(f"  Fallback LLM : {OLLAMA_MODEL_NAME}")
#     print(f"  Tables       : {len(found)}  |  Columns: {total_vals}  |  Relationships: {total_rels}")
#     print(f"  Value index  : {'enabled' if value_retriever else 'disabled'}")
#     print(f"  SQL critique : {'on' if ENABLE_SQL_CRITIQUE else 'off'}  |  Row cap: {ROW_CAP}")
#     print("  Type 'exit' or 'q' to quit.")
#     print("=" * 70)
