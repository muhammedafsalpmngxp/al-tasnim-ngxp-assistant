...
class IntentParser:
    ...
    # Max columns to show per table in the prompt.
    # Large tables (e.g. WMR with 128 cols) would otherwise produce a 17k-char
    # prompt that makes qwen3:8b extremely slow and truncate its JSON output.
        ...
        for table in settings.ALLOWED_TABLES:
            if self.schema:
                try:
                    ...
                    all_cols = schema["column_list"]
                    shown = all_cols[: self._MAX_COLS_PER_TABLE]
                    omitted = len(all_cols) - len(shown)
                    col_strs = [
                    ...
                    lines.append("Columns: " + ", ".join(col_strs) + suffix)
                    continue
                except Exception as e:
...
"rig", "drilling rig", "rig number",
"assigned to rig", "wells on rig",
"wells under rig"                        → column RIG  (table 2026_Well_Delivery_Scope_Well_Type)
...
User: "How many wells are assigned to rig 149?"
Thinking: topic=wells count for a specific rig → table=2026_Well_Delivery_Scope_Well_Type, filter RIG=149, aggregate count(*).
Output:
{{
  "table": "2026_Well_Delivery_Scope_Well_Type",
  "columns": ["Well_ID", "RIG"],
  "filters": [{{"col": "RIG", "op": "=", "value": 149}}],
  "aggregate": {{"func": "count", "group_by": []}},
  "order_by": [],
  "limit": 1,
  "natural_language": "How many wells are assigned to rig 149?",
  "clarification_needed": false,
  "clarification_question": null
}}
